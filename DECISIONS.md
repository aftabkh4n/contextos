...
|------------|---------------------------------------------|------------------------------------------------------------------------------------------------------------|
| 2026-05-22 | .nupkg excludes ONNX model and all native runtime libs | The tool package would be 209 MB because `PackTool` globs the entire publish dir; OnnxRuntime ships native libs for all platforms (~180 MB) and the model is 86 MB. A target deletes these from the publish dir before PackTool runs during pack only (`_IsPacking=true`). Result: 1.9 MB nupkg. Self-contained binaries (published with `--self-contained -p:PublishSingleFile=true`) are unaffected and include all assets. Tool-install users who need ONNX should download the self-contained binary. |
